...
/*GET Form5B LIST*/
async getForm5BListByUser(params: { 
  uid: number; 
  role: number; 
  zone_id?: string; 
}) {
...
/*GET Editable Form5B*/
async getEditableForm5BByUser(params: { uid: number; role: number }) {
  ...
  if (!filedSocieties || filedSocieties.length === 0) {
    return {
      form4: {
        id: form4.id,
        district_name: cleanText(form4.district_name),
...
/*PUT Edit Form5B*/
async editForm5B(payload: any) {

  const { uid, role, societies, candidates } = payload;
