...
async submitMembers(payload: any) {
  ...
  for (const m of members) {
    if (!validFiledSocIds.includes(Number(m.form4_filed_soc_id))) {
      throw new Error(
        `Invalid form4_filed_soc_id ${m.form4_filed_soc_id} for latest Form4`
      );
    }
  }
...
await prisma.form5.createMany({
  data: members.map((m) => ({
    ...
    member_name: cleanText(String(m.member_name ?? "")) || "",
    aadhar_no: cleanText(String(m.aadhar_no ?? "")) || "",
    is_active: true,
  })),
});
